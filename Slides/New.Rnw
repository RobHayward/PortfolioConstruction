...
% Title and author information
  \title{New Developments and the future}
\author{Rob Hayward}
\begin{document}
...
\item Questions 
\begin{itemize}
\item Most important objectivce
\item Changes to deal wtih low returns
...
There are two main structure for funds
\begin{itemize}[<+-| alert@+>]
\pause
...
\begin{frame}{Decomposition of portolio return}
\begin{tikzpicture} 
%\draw[very thin, color = gray](0, 0) grid (12, 6);
%\draw (0, 5) to (0, 4) to (3, 4) to (3, 5) to (0, 5);
%\node at (0, 4.5) [right] {\scriptsize True passive Market unobserved};
%\draw (4, 5) to (4, 4) to (7, 4) to (7, 5) to (4, 5);
%\draw (8, 5) to (8, 4) to (11, 4) to (11, 5) to (8, 5);
%\draw (8, 2) to (8, 1) to (11, 1) to (11, 2) to (8, 2);
...
\path [line] (four) -- (five);
\node at (3, 5.5) [align = center]{\scriptsize Choice of investible \\ \scriptsize benchmark};
