...
\begin{itemize}[<+-| alert@+>]
\pause
\item Capitalisation-weighted indices are \textbf{NOT} equivalent to the \emph{market porttfolio}
\begin{itemize}
...
\item Use investment anomolies or \emph{factors}
\end{itemize}
\end{frame}
...
Arnott, Hsu and More tested four indices
\begin{itemize}[<+-| alert@+>]
\pause
\item Book value
...
\includegraphics[scale = 0.4]{Main-Street}
\end{column}
\begin{column}{0.48\linewidth}
\includegraphics[scale = 0.25]{Wall-Street}
\end{column}
\end{columns}
\end{frame}
...
\item Portfolio re-balanced one a year (at end of year)
...
Portfolio/index & 1980s & 1990s & 2000-04\\
\hline
...
\begin{itemize}[<+-| alert@+>]
\item This is not \emph{data mining}
\begin{itemize}
...
\item Reults hold in rising and falling rate environment
\end{itemize}
...
\end{itemize}
\item Correlation with the returns and volatility of the capitaisation-weighted index is high
\end{itemize}
\end{frame}
...
\begin{center}
\begin{tabular}{l r r r}
Portfolio/index & Value-added & Ann Std Dev &  Sharpe\\
\hline
...
Aim to build portfolios based on particular \emph{factors}
\begin{itemize}[<+-| alert@+>]
\pause
\item value
\item Avoiding growth
\end{itemize} 
\puse
Are already captured by Adustments to get the size of the firm. 
\begin{itemize}[<+-| alert@+>]
\pause
\item Small capitalisation
\item Momentum
\item Others
\end{itemize}
May have to be captured explicitly
\end{frame}
...
\item \emph{Crowded?} - concentrated in utilties
\end{itemize}
\end{frame}
...
\item Leveage bond holdings to make the risk comparable to that of stocks. 
\end{itemize}
\end{frame}

\begin{frame}{Risk partity exampels}
\begin{table}
\begin{center}
\rowcolors{1}{OliveGreen!20}{OliveGreen!5}
\begin{tabular}{l  r r}
Performance & 60/40 stock-bonds & Risk parity\\
\hline
...
There are a numnber of criticism of Smart Beta (ironically, these have recently 
come from \textbf{Research Affiliates}). 
\begin{itemize}[<+-| alert@+>]
\pause
\item Data mining
\item Exploitation of the inefficiency should remove that ineffiicnecy
