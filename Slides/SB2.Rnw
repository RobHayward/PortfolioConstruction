...
\begin{frame}{Assets as bundles of risk}
A standard pension fund may hold the following assets
...
\item Maturiting matching
\item Are liabilties bond-like or equity-like?
\end{itemize}
\item Statistical methods
\begin{itemize}
\item Principal components (PCA)
\item Seeks to identify orthoghonal factors that contribute most to the variation of the portfolio
\end{itemize}
\end{itemize}
\end{frame}
