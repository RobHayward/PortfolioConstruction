...
\item Are these effects (value, growth etc) ineffiiciencies or risk factors? 
\end{itemize}
\end{frame}
...
\item List needed////
\end{itemize}
\item However, individual managers may be succssful
\item Hedge funds have come under recent scrutiny and criticism
\end{itemize}
\end{frame}
