...
\item Active management fees are down to 63bp from 100bp in 2000
...
\item In the environment of very low rates, there is particular intrest in floating rate, short maturity and emerging market bond ETFs
\end{itemize}
\end{frame}
...
\begun{frame}{Statistical methods}
\begin{itemize}[<+-| alert@+>]
...
\item It may find that different assets have very similar risk factors
\end{itemize}
\end{frame}
...
\begin{itemize}[<+-| alert@+>]
\item Low volatility introduced in 2011
\item 
\item \emph{Crowded?} - concentrated in utilities
% need to complete this slides.  What special ETFs have emerged.
\end{itemize}
\end{frame}
...
plot(da$Date, da[,2], type = 'l', main = "US Official Interest Rate", col = 'brown', 
     xlab = "Year", ylab = "Rate")
@
\end{frame}
