...
\includegraphics[scale = 0.2]{Rumsfeld}
\end{textblock*}
...
\pause
\draw [fill = red] (3.3, 1.8) circle [radius = 0.05];
\node at (3.3, 1.8) [right] {\scriptsize Growth};
\end{tikzpicture}
\end{frame}
...
\item Only 16.9\% of US large-growth managers were able to beat their passive conterparts over the 10 year period
...
\item Hordahl et al. (2016): 2005-15 100bp rise in US yield curve associated with a 70bp to 80bp rise in  
 \item Reduced liquity due to regulation of investment banks (rebalancing more expensive)
